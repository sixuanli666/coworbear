...
# 同一行：模型选择 + 平移周数
col_model, col_shift, col_blank = st.columns([3, 2, 3])

with col_model:
    score_col = st.selectbox(
        '选择模型（未来1周、4周、8周、12周、16周、20周）',
        score_candidates,
        index=16 if score_candidates else None,
        key='main_score'
    )
...
# 参数设置 - 图1和图2参数并排显示
# ===== 1. 选择行业 & MA窗口 =====
all_inds = sorted({c[:-4] for c in df_ind.columns if c.endswith("_pct")}) if not df_ind.empty else []
default_inds = [x for x in ["电子", "通信", "传媒", "计算机"] if x in all_inds]

col_param1, col_param2 = st.columns(2)

with col_param1:
    st.subheader("2.2图1行业成交额占比·参数")

...
IC_df=IC_df.rename(columns={'horizon_w':'窗口期（未来几周)','factor':'因子','type':'因子类型','N':'样本数量','pearson':'pearson相关系数IC','spearman_ic':'spearman相关系数IC'})
st.write(IC_df)
...
st.caption('下图展示了牛熊因子模型输出的总分在历史中与指数走势的对比，可以根据IC大小选择不同未来周收益率模型输出分数，一般IC越大模型预测效果越好，此外可以向右平移分数线查看当前预测指数走势，例如若选择预测未来16周指数收益率模型，可向右平移分数线16周，查看当前分数走势')
...
#     st.write("300指数（剔除了金融股）的股息率与十年期国债收益率的比较。股息率是指股票的年度股息除以股价，而十年期国债收益率则是债券投资的回报率。图中展示了这两者的变化趋势。蓝色虚线表示平均值，+1倍和-1倍标准差，图示了股息率与国债收益率的相对收益率情况。")
st.caption('300指数（剔除了金融股）的股息率与十年期国债收益率的比较。股息率是指股票的年度股息除以股价，而十年期国债收益率则是债券投资的回报率。图中展示了这两者的变化趋势。蓝色虚线表示平均值，+1倍和-1倍标准差，图示了股息率与国债收益率的相对收益率情况。(已增加上证综指)')   
...
#     """)
# st.caption("本图展示了A股的风险溢价（E/P，即市盈率倒数）与十年期国债收益率的时序关系。风险溢价表示股票市场的收益率相对于国债的超额回报，E/P 值越高，表示股票的风险溢价越高。通过计算市盈率倒数和股票的市值加权，得出加权的风险溢价。图中展示了A股风险溢价随时间的变化趋势，以及其相对于十年期国债收益率的波动范围。
#     蓝色虚线表示风险溢价的均值，+1倍和-1倍标准差，图示了股市的波动性和国债收益率的变化关系。")

# st.markdown("""
# <div style="line-height: 1.4; color: #808080;">
# 本图展示了A股的风险溢价（E/P，即市盈率倒数）与十年期国债收益率的时序关系。风险溢价表示股票市场的收益率相对于国债的超额回报，E/P 值越高，表示股票的风险溢价越高。通过计算市盈率倒数和股票的市值加权，得出加权的风险溢价。图中展示了A股风险溢价随时间的变化趋势，以及其相对于十年期国债收益率的波动范围。<br>
# 蓝色虚线表示风险溢价的均值，+1倍和-1倍标准差，图示了股市的波动性和国债收益率的变化关系。

# </div>
# """, unsafe_allow_html=True)

st.caption('本图展示了A股的风险溢价（E/P，即市盈率倒数）与十年期国债收益率的时序关系。风险溢价表示股票市场的收益率相对于国债的超额回报，E/P 值越高，表示股票的风险溢价越高。通过计算市盈率倒数和股票的市值加权，得出加权的风险溢价。图中展示了A股风险溢价随时间的变化趋势，以及其相对于十年期国债收益率的波动范围。蓝色虚线表示风险溢价的均值，+1倍和-1倍标准差，图示了股市的波动性和国债收益率的变化关系。')   
...
#     """)
st.caption('跟踪“大盘vs小盘”等多空组合的收益和净值，在任意周期（日/月/季）上显示谁在持续占优，并用均值±σ告诉你这个风格强弱是否已经走到极端、有没有到该防风格反转/调仓的时点。')   
